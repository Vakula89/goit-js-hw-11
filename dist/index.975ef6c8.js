...
const onKeyDown = (ev)=>{
    ...
    (0, _fetchCountries.fetchCountries)(searchStr).then((data)=>{
        if (data.length > 10) {
        ...
        if (data.length < 10 && data.length > 1 && index === -1) {
            countryInfo.innerHTML = "";
            countryList.innerHTML = data.map((el)=>`<li><img src="${el.flags.svg}" alt="flag" width="20"> <span>${el.name.official}</span></li>`).join("");
            return;
        }
        if (index === -1) index = 0;
        const country = data[index];
        countryList.innerHTML = "";
        countryInfo.innerHTML = `<img src="${country.flags.svg}" alt="flag" width="100">` + `<h1>${country.name.official}</h1>` + `<dl>` + `<dt>Capital:</dt><dd>${country.capital.join(", ")}</dd>` + `<dl><dt>Population:</dt><dd>${country.population}</dd>` + `<dl><dt>Languages:</dt><dd>${Object.keys(country.languages).map((key)=>country.languages[key]).join(", ")}</dd>` + `</dl>`;
    }).catch((error)=>{
        countryList.innerHTML = "";
        countryInfo.innerHTML = "";
        ...
        (0, _notiflixDefault.default).Notify.failure("Oops, unknown error happens - look in console");
        console.log(error);
    });
};
searchBox.addEventListener("keydown", (0, _lodashDebounceDefault.default)(onKeyDown, DEBOUNCE_DELAY));

},{"./css/styles.css":"1CY4s","./fetch.Countries":"hZ3Tp","notiflix":"5WWYd","notiflix/dist/notiflix-3.2.5.min.css":"jvRD8","lodash.debounce":"3JP5n","@parcel/transformer-js/src/esmodule-helpers.js":"gkKU3"}],"1CY4s":[function() {},{}],"hZ3Tp":[function(require,module,exports) {
var parcelHelpers = require("@parcel/transformer-js/src/esmodule-helpers.js");
...
},{}],"jvRD8":[function() {},{}]},["1RB6v","8lqZg"], "8lqZg", "parcelRequired7c6")
