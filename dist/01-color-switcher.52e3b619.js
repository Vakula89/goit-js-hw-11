...
function bgColorChanger() {
    ...
    if (btnStart.getAttribute("disabled")) setTimeoutHandler = setTimeout(()=>{
        bgColorChanger();
    }, 1000);
}
btnStart.addEventListener("click", (ev)=>{
    if (!ev.target.getAttribute("disabled")) ev.target.setAttribute("disabled", "disabled");
    btnStop.removeAttribute("disabled");
    bgColorChanger();
});
btnStop.addEventListener("click", (ev)=>{
    if (!btnStart.getAttribute("disabled")) return;
    if (!ev.target.getAttribute("disabled")) ev.target.setAttribute("disabled", "disabled");
