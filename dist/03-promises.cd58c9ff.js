...
function validate(form1) {
    const validators = {
        amount: (value)=>{
            ...
            if (!valid) (0, _notiflixNotifyAio.Notify).warning(`amount must be integer > 0`);
            return valid;
        },
        delay: (value)=>{
            let valid = !isNaN(value) && value >= 0;
            if (!valid) (0, _notiflixNotifyAio.Notify).warning(`delay must be integer >= 0`);
            return valid;
        },
        step: (value)=>{
            let valid = !isNaN(value) && value >= 0;
            if (!valid) (0, _notiflixNotifyAio.Notify).warning(`step must be integer >= 0`);
            return valid;
        }
    };
    return Object.keys(validators).reduce((acc, key)=>{
        let v = parseInt(form1.elements[key].value, 10);
        let valid = validators[key](v);
        return acc && valid;
    }, true);
}
function doPromises(data1) {
    let { position: position1 , form: form2  } = data1;
    const elems = form2.elements;
    const amount = parseInt(elems["amount"].value, 10);
    const delay1 = parseInt(elems["delay"].value, 10);
    const step = parseInt(elems["step"].value, 10);
    let currDelay = step;
    form2.setAttribute("pending", "true");
    currDelay = delay1;
    const unblocking = (position)=>{
        if (position === amount) form2.removeAttribute("pending");
    };
...
form.addEventListener("submit", (ev)=>{
    ev.preventDefault();
    if (ev.currentTarget.getAttribute("pending")) return;
